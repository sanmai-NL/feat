...
#' @slot IS_ALL_NGRAMS_UP_TO_N Extract all \eqn{n}-grams with \eqn{n \in \{1,...,N\}}? If false, extract all \eqn{N}-grams with \eqn{N=}\code{N_I}.
#' @slot NAME_STR \emph{private} \code{'STRCT'}.
#' @slot N_I A scalar integer \eqn{N}.
#' @slot N_STRIDES_TO_TAKE_I How long should trails be, at most? If the current arc in the current trail goes out to a terminal vertex yet has not reached this length, the trail is accumulated as is.
#' @slot SCALE_ITERATIONS_I The weight to scale the number of iterations taken for random sampling of trails in a linguistic network by, a multiplier of the number of arcs in the network.
...
StrctFeatureRepresentation <-
    methods::setClass(
        ...
        slots=base::list(
            ...
            IS_ALL_NGRAMS_UP_TO_N='logical',
            N_I='integer',
            N_STRIDES_TO_TAKE_I='integer',
            SCALE_ITERATIONS_I='integer',
            SRILM_NGRAMCOUNT_PARAMETERS_CVEC='character',
            SRILM_NGRAM_PARAMETERS_CVEC='character',
            XPATH_STR='character'),
        prototype=base::list(
            ARC_CATEGORIES=NULL,
            IS_ALL_NGRAMS_UP_TO_N=NULL,
            N_I=NULL,
            N_STRIDES_TO_TAKE_I=NULL,
            SCALE_ITERATIONS_I=NULL,
