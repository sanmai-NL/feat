...
ARC_CATEGORIES_DT_COLUMN_CLASSES <-
    base::c(
        base::character(),
        base::character())
...
#' @export
input_write_annotation_table_template <- function(DOCUMENTS_DIRS_PATH_STR=NULL, OUTPUT_DIR_PATH_STR=NULL, ANNOTATION_TABLE_FILE_PATH_STR=base::file.path(output_dir_path_str, 'annotation_table.tsv')) {
    ...
    return(ANNOTATION_TABLE_FILE_PATH_STR)
}
...
input_read_annotation_table <- function(ANNOTATION_TABLE_FILE_PATH_STR=NULL, DOCUMENTS_DIR_PATH_STR=NULL, SEGMENT_TYPES_CVEC=NULL) {
    ...
    annotation_dt <-
        data.table::fread(
            ...
            encoding='UTF-8',
            drop='comments',
            colClasses=ANNOTATION_DT_COLUMN_CLASSES,
...
input_read_arc_categories_table <- function(ARC_CATEGORIES_FILE_PATH_STR=NULL) {
    ...
    ARC_CATEGORY_DT <-
        data.table::fread(
            ...
            sep='\t',
            encoding='UTF-8',
            colClasses=ARC_CATEGORIES_DT_COLUMN_CLASSES,
            stringsAsFactors=TRUE)
